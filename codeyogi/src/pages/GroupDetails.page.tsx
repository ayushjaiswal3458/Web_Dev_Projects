...
import {   groupLoadingSelector, groupOneErrorSelector, groupSelector, nextIdSelector, prevIdSelector } from "../selectors/groups.selectors";
...
const GroupDetailsPage: React.FC<Props>=(props) => {
    ...
    if(groupError){
        return <div><Alerts 
        strong="" theme="blue" className="z-30 mt-20 ml-10 w-148.8985" >{groupError}</Alerts></div>;
    }
    
    return (
        <div className=" mt-20 mx-4 rounded-lg bg-gray-400 w-full  ">
            { groupLoading && <Alerts strong="Group Loading " theme="indigo" >Please wait...</Alerts>}
            <div className="rounded-lg shadow-lg bg-white m-4 p-4">
            <h1 className="text-xl font-semibold ">Group information </h1>
            {group && <div className="my-2 flex">
                <img
            ...
            <button onClick={() =>
                dispatch(fetchOneGroup(prevId!))
            }><GrFormPrevious className="p-2 rounded-full text-black w-10 h-10 bg-gray-200 shadow-2xl" /></button>
            </Link>
            <Link to={`/groups/${nextId}`}>
                
            <button onClick={() =>
                dispatch(fetchOneGroup(nextId!))
            }><GrFormNext className="p-2 rounded-full text-black w-10 h-10 bg-gray-200 shadow-2xl" /></button>
            </Link>
